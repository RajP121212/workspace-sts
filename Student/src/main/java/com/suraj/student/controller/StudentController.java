...
public class StudentController {
	...
	public List<Course> retrieveCoursesForStudent(@PathVariable String studentId) {
		return studentService.retrieveCoursesGivenStudentId(studentId);
	}

	//
	@GetMapping("/students/{studentId}/courses/{courseId}")
	public Course retrieveDetailsForCourse(@PathVariable String studentId, @PathVariable String courseId) {
		return studentService.retrieveCourseGivenStudentIdAndCourseId(studentId, courseId);
	}

	// If we use ResponseEntity with any Controller Method, we are just allowing
	// that method to..
	// ..specify ResponseBody and ResponseHeader
